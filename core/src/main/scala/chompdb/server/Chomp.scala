...
def downloadDatabaseVersion(database: Database, version: Long) = {
	...
	override def mapReduce[T: TypeTag](catalog: String, database: String, keys: Seq[Long], 
      mapReduce: MapReduce[ByteBuffer, T]) = {
    ...
    parMap(keysToNodes) map { case (node, ids) =>
      val serializedResult = nodeProtocol(node).mapReduce(catalog, database, version, 
          ids, serializeMapReduce(mapReduce))
