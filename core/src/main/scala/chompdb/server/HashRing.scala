...
class HashRing(chomp: Chomp) {
  ...
  def getNodesForShard(shardId: Int): Set[Node] = {
    ...
    nodeMap.size match {
      ...
      case _ => {
        def assembleNodeSet(nodeSet: Set[Node], count: Int): Set[Node] = count match {
          case c if c < 0 => {
          ...
          case c if c > 0 => {
            ...
            assembleNodeSet(nodeSet + nodeMap.get(nearestNodeHashValue), c - 1)
          }
        }

        assembleNodeSet(Set.empty[Node], chomp.replicationFactor)
      }
    } 
  }
